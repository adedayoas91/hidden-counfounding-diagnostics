...
This module reimplements the hot paths of :class:`GcStar`
(``markovianity_diagnostic.core.causalised-GC``) without changing the inference
semantics or the output format. It subclasses the estimator and overrides only
the two expensive passes, so ``fit`` / ``get_connectivity_matrix`` and the whole
graph-construction logic are inherited unchanged (guaranteed output parity).
